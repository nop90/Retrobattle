...
//		      if(Fuzz *f = dynamic_cast<Fuzz *>(*nmy))
		      if((*nmy)->checkName("Fuzz"))
			{
			  Fuzz *f = static_cast<Fuzz *>(*nmy);
			  f->stun();
			}
		      else
			{
...
//      if(LaserBeam *lb = dynamic_cast<LaserBeam *>(*b))
      if((*b)->checkName("LaserBeam"))
	{
	  LaserBeam *lb = static_cast<LaserBeam *>(*b);
...
//	  else if(dynamic_cast<BountyHunter *>(*it))
	  else if((*it)->checkName("BountyHunter"))
	    {
