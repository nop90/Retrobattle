...
$HOME_DIR/$CONFIG_DIR/topscore.dat
$HOME_DIR//$CONFIG_DIR/keyconfig.dat
...
//  if(getenv("HOME") != 0)
//    {
      sprintf(filename, "/%s/%s/%s.dat", HOME_DIR, CONFIG_DIR, file);
/*    }
  else
    {
...
FileManager::makeConfigDirIfMissing()
{
  ...
  sprintf(dirname, "/%s", HOME_DIR);
  mkdir(dirname, 0777);
  sprintf(dirname, "/%s/%s", HOME_DIR, CONFIG_DIR);
  mkdir(dirname, 0777);
